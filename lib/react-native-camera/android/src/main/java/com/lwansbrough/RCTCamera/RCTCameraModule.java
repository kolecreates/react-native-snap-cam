...
public class RCTCameraModule extends ReactContextBaseJavaModule
    ...
    private Stack<File> mVideoClips;
    private Stack<File> mVideoClipPreviewImages;
    ...
    public RCTCameraModule(ReactApplicationContext reactContext) {
        ...
        mVideoClips = new Stack<File>();
        mVideoClipPreviewImages = new Stack<File>();
    }
    ...
    private Throwable prepareMediaRecorder(ReadableMap options) {
        ...
        mMediaRecorder.setVideoEncodingBitRate(6900000);
    ...
    private void releaseMediaRecorder() {
        ...
        if (duration < 1000) {
          removeClipPreviewImage();
          mRecordingPromise.reject("RECORD_MIN_LENGTH");
        ...
        switch (mRecordingOptions.getInt("target")) {
            ...
            case RCT_CAMERA_CAPTURE_TARGET_DISK:
                response.putString("path", Uri.fromFile(mVideoFile).toString());
                response.putString("preview", Uri.fromFile(mVideoClipPreviewImages.peek()).toString());
                response.putInt("duration", (int) duration);
                mRecordingPromise.resolve(response);
        }
    ...
    @ReactMethod
    public void unlock(ReadableMap options){
      mCamera = RCTCamera.getInstance().acquireCameraInstance(options.getInt("type"));
      if(mCamera != null){
        mCamera.unlock();
      }
    ...
    @ReactMethod
    public void lock(ReadableMap options){
      mCamera = RCTCamera.getInstance().acquireCameraInstance(options.getInt("type"));
      if(mCamera != null){
        mCamera.lock();
      }
    ...
    public void savePicture(final Promise promise){
      try{
        ...
        e.printStackTrace();
        promise.reject("Error writing to file.");
      }
    ...
    public void deleteVideoClip(){
      if(!mVideoClips.empty()){
        mVideoClips.pop().delete();
        if(!mVideoClipPreviewImages.empty()){
          mVideoClipPreviewImages.pop().delete();
        }
      }
    }

    @ReactMethod
    public void getVideoClip(final Promise promise){
      if(!mVideoClips.empty()){
        final WritableMap response = new WritableNativeMap();
        response.putString("path", Uri.fromFile(mVideoClips.peek()).toString());
        response.putString("preview", Uri.fromFile(mVideoClipPreviewImages.peek()).toString());
        promise.resolve(response);
      }else {
        promise.reject("No Clip In Stack");
      }
    }

    @ReactMethod
    public void stitchVideoClips(final Promise promise){
      AsyncTask.execute(new Runnable() {
          @Override
          public void run() {

          File file = getTempMediaFile(MEDIA_TYPE_VIDEO);
          boolean success = RCTCameraUtils.stitchVideoClips(mVideoClips,file , false);
          if(success){
            clearVideoClips();
            mVideoClips.push(file);
            final WritableMap response = new WritableNativeMap();
            response.putString("path", Uri.fromFile(file).toString());
            promise.resolve(response);
          }else{
            promise.reject("Error stitching or writing to file.");
          }
        }
      });
    }
    ...
    public void takeScreenshot(final Promise promise){
      AsyncTask.execute(new Runnable() {
          @Override
          public void run() {
            File pictureFile = previewToImageFile();
            if (pictureFile == null) {
                promise.reject("Error taking screenshot.");
                return;
            }
    ...
    public File previewToImageFile(){
      MutableImage image;
      RCTCameraView view = RCTCameraView.getInstance();
      RCTCameraViewFinder finder = view.getViewFinder();
      image = new MutableImage(finder.getBitmap());
      File pictureFile = getTempMediaFile(MEDIA_TYPE_IMAGE);
      try {
         image.writeDataToFileSimple(pictureFile, 100);
         return pictureFile;
      } catch (IOException e) {
          return null;
      }
    ...
    public void pushNewClipPreviewImage(){
      File temp = previewToImageFile();
      if(temp != null){
        mVideoClipPreviewImages.push(temp);
      }
    }

    public void removeClipPreviewImage(){
      if(!mVideoClipPreviewImages.empty()){
        mVideoClipPreviewImages.pop().delete();
      }
    }
    ...
    public void capture(final ReadableMap options, final Promise promise) {
        if (RCTCamera.getInstance() == null) {
         ...
         final int orientation = options.hasKey("orientation") ? options.getInt("orientation") : RCTCamera.getInstance().getOrientation();
